...
- When in doubt, ask.
- **Rule of One:** Standardize on a single browser session. Always prioritize the **Collaborative Browser (Novian Bridge)** to use Andrei's active session. Avoid the isolated `browser_subagent` as it creates "dual launch" instances that leak RAM and lack persistence.
